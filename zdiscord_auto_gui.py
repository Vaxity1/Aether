...
from UI_Templates.header_graphic import HeaderGraphic
...
def log_error(error_type: str, error_msg: str, context: str, resolution: str | None = None) -> None:
...
def log_session(summary: str) -> None:
    with open(SESSION_LOG, "a", encoding="utf-8") as f:
        timestamp = datetime.now().isoformat()
        emoji = random.choice(["✅", "🟢", "🔵", "✨", "🚀", "💡"])
        f.write(f"[{timestamp}] {emoji} {summary}\n" + "-"*60 + "\n")

THEMES = {"Dark": "dark-blue", "Light": "green", "Discord": "dark-blue"}
LANGUAGES = {"English": "en", "Español": "es"}

# --- Modern Icon Loader ---
def load_icon(path: str, size: tuple[int, int] = (24, 24)) -> Any:
...
class Tooltip:
    ...
    def show_tip(self, event: Any = None) -> None:
        ...
        label = tk.Label(tw, text=self.text, justify=tk.LEFT,
                        background="#181A20", foreground="#00fff7",
    ...
    def hide_tip(self, event: Any = None) -> None:
        if self.tipwindow:
            ...
            self.tipwindow = None
    def _fade_in(self, tw: tk.Toplevel) -> None:
        if self.fade < 1.0:
            self.fade += 0.1
            tw.attributes("-alpha", self.fade)
            tw.after(20, lambda: self._fade_in(tw))
        else:
            tw.attributes("-alpha", 1.0)

# --- Main Application Class ---
class VaxityAutoTyper:
    ...
    def build_gui(self):
        # Header Frame (now imported)
        self.header_graphic = HeaderGraphic(self.root, title="VaxityAutoTyper")
        # Remove old header_frame, icon_label, title_label

        # --- Topbar: Theme, Language, Update, Discord Status ---
        topbar = ctk.CTkFrame(self.root, height=44, fg_color="#181A1B")
        topbar.pack(side=ctk.TOP, fill=ctk.X, pady=(0, 2))
        theme_label = ctk.CTkLabel(topbar, text="Theme:", font=("Segoe UI", 12), text_color="#99AAB5")
        theme_label.pack(side=ctk.LEFT, padx=(16,2))
        ...
        lang_label = ctk.CTkLabel(topbar, text="Language:", font=("Segoe UI", 12), text_color="#99AAB5")
        ...
        update_btn = ctk.CTkButton(topbar, text="Check for Updates", width=140, command=self.check_update, fg_color="#7289DA", hover_color="#5865F2", font=("Segoe UI", 12))
        update_btn.pack(side=ctk.LEFT, padx=(24,2))
        self.discord_status = ctk.CTkLabel(topbar, text="Discord: Disconnected", font=("Segoe UI", 12, "bold"), text_color="#F04747")
        self.discord_status.pack(side=ctk.RIGHT, padx=16)
        Tooltip(self.discord_status, "Shows Discord connection status.")

        # --- Sidebar Navigation ---
        sidebar = ctk.CTkFrame(self.root, width=68, fg_color="#181A1B")
        sidebar.pack(side=ctk.LEFT, fill=ctk.Y, padx=(0, 2))
        main_icon = ctk.CTkButton(sidebar, text="💬", width=54, height=54, fg_color="#23272A", command=lambda: self.notebook.select(0), corner_radius=16)
        main_icon.pack(pady=(24, 12))
        Tooltip(main_icon, "Main Panel")
        template_icon = ctk.CTkButton(sidebar, text="📄", width=54, height=54, fg_color="#23272A", command=lambda: self.notebook.select(1), corner_radius=16)
        template_icon.pack(pady=12)
        Tooltip(template_icon, "Templates")
        analytics_icon = ctk.CTkButton(sidebar, text="📊", width=54, height=54, fg_color="#23272A", command=self.show_analytics, corner_radius=16)
        analytics_icon.pack(pady=12)
        Tooltip(analytics_icon, "Analytics Dashboard")
        help_icon = ctk.CTkButton(sidebar, text="❓", width=54, height=54, fg_color="#23272A", command=self.show_help, corner_radius=16)
        help_icon.pack(pady=(12, 0))
        Tooltip(help_icon, "Help & Onboarding")

        # --- Main Card (GroupBox) ---
        self.card = ctk.CTkFrame(self.root, corner_radius=25, fg_color="#23272A", border_width=0, border_color="#23272A")
        self.card.pack(side=ctk.LEFT, fill=ctk.BOTH, expand=True, padx=0, pady=0)
        # Make card less visible (almost transparent)
        self.card.configure(fg_color="#23272A", border_width=0)

        # --- Tabbed Interface ---
        style = ttk.Style()
        style.theme_use('default')
        style.configure('TNotebook.Tab', background='#181A1B', foreground='#99AAB5', font=('Segoe UI', 13, 'bold'), padding=[16, 8])
        style.map('TNotebook.Tab', background=[('selected', '#23272A')], foreground=[('selected', '#7289DA')])
        ...
        self.notebook.pack(fill=ctk.BOTH, expand=True, padx=12, pady=12)

        # --- Main Tab Layout ---
        parent = self.tab_main
        self.header = ctk.CTkLabel(parent, text="💬 VaxityAutoTyper", font=("Segoe UI Black", 30, "bold"), text_color="#00FFD0")
        self.header.pack(pady=(22, 10))
        self.shine = ctk.CTkProgressBar(parent, width=340, height=10, corner_radius=8, progress_color="#00FFD0", fg_color="#181A1B")
        self.shine.pack(pady=(0, 22))
        self.shine.set(0.0)
    ...
    # --- Enhanced Neon Loading Bar Patterns ---
    def animate_loading_bar(self):
        ...
        # Expanded and more random styles
        self.loading_bar_styles = [
            'neon', 'dashed', 'wave', 'block', 'vertical_art1', 'vertical_art2',
            'dots', 'zigzag', 'rainbow', 'matrix', 'sparkle', 'pulse', 'glow', 'bars', 'shards'
        ]
        style = self.loading_bar_styles[self.loading_bar_style_idx]
        shimmer_width = 40
        self.loading_bar_shimmer_pos = (self.loading_bar_shimmer_pos + random.randint(2, 6)) % (width + shimmer_width)
        flash = self.loading_bar_flash and (time.time() - self.last_flash_time < 0.25)
        base_color = random.choice(["#00FFD0", "#7289DA", "#43B581", "#F04747", "#FFD700"]) if not flash else "#ffffff"
        glow_color = random.choice(["#00FFD0", "#7289DA", "#43B581", "#F04747", "#FFD700"]) if not flash else "#ffffff"
        # Draw different styles (expanded)
        if style == 'neon':
            self.loading_bar_canvas.create_line(0, height//2, width, height//2, fill=base_color, width=7, capstyle="round")
            for i in range(1, 7):
                self.loading_bar_canvas.create_line(0, height//2, width, height//2, fill=glow_color, width=7+2*i, capstyle="round", stipple="gray50")
            ...
            for x in range(0, width, 24):
                self.loading_bar_canvas.create_line(x, height//2, x+12, height//2, fill=base_color, width=6, capstyle="round")
        elif style == 'wave':
            points = []
            for x in range(0, width, 6):
                y = height//2 + int(7 * (1 + math.sin((x + self.loading_bar_shimmer_pos) * 0.07)))
                points.append((x, y))
            for i in range(len(points)-1):
                self.loading_bar_canvas.create_line(points[i][0], points[i][1], points[i+1][0], points[i+1][1], fill=base_color, width=4)
        elif style == 'block':
            for x in range(0, width, 16):
                self.loading_bar_canvas.create_rectangle(x, 2, x+10, height-2, fill=base_color, outline="")
        elif style == 'vertical_art1':
            for x in range(0, width, 12):
                char = random.choice(['│','┃','║','▌','▐','▎','▍','▊','█','▇','▆','▅','▄'])
                self.loading_bar_canvas.create_text(x+6, height//2, text=char, font=("Consolas", 16), fill=base_color)
        elif style == 'vertical_art2':
            for x in range(0, width, 14):
                char = random.choice(['╷','╵','╹','╻','╽','╿','┇','┋','┃','╳','╱','╲'])
                self.loading_bar_canvas.create_text(x+7, height//2, text=char, font=("Consolas", 18), fill=glow_color)
        elif style == 'dots':
            for x in range(0, width, 10):
                self.loading_bar_canvas.create_oval(x, height//2-3, x+6, height//2+3, fill=base_color, outline="")
        elif style == 'zigzag':
            for x in range(0, width-10, 10):
                y1 = height//2 + (x%20-10)//2
                y2 = height//2 - (x%20-10)//2
                self.loading_bar_canvas.create_line(x, y1, x+10, y2, fill=base_color, width=3)
        elif style == 'rainbow':
            colors = ["#FF0000", "#FF7F00", "#FFFF00", "#00FF00", "#0000FF", "#4B0082", "#9400D3"]
            for i, x in enumerate(range(0, width, 20)):
                self.loading_bar_canvas.create_rectangle(x, 2, x+18, height-2, fill=colors[i%len(colors)], outline="")
        elif style == 'matrix':
            for x in range(0, width, 14):
                char = random.choice(['0','1'])
                self.loading_bar_canvas.create_text(x+7, height//2, text=char, font=("Consolas", 18), fill="#00FF00")
        elif style == 'sparkle':
            for _ in range(width//12):
                x = random.randint(0, width)
                y = random.randint(0, height)
                self.loading_bar_canvas.create_oval(x, y, x+3, y+3, fill="#FFD700", outline="")
        elif style == 'pulse':
            pulse_color = "#00FFD0" if int(time.time()*2)%2==0 else "#7289DA"
            self.loading_bar_canvas.create_line(0, height//2, width, height//2, fill=pulse_color, width=10, capstyle="round")
        elif style == 'glow':
            for i in range(1, 8):
            ...
            for x in range(0, width, 18):
                self.loading_bar_canvas.create_rectangle(x, 2, x+8, height-2, fill=base_color, outline="")
        elif style == 'shards':
            for x in range(0, width, 20):
                y1 = random.randint(0, height//2)
                y2 = random.randint(height//2, height)
                self.loading_bar_canvas.create_line(x, y1, x+10, y2, fill=base_color, width=2)
        self.root.after(70, self.animate_loading_bar)
    ...
    def load_quick_template(self, tname):
        ...
        try:
            ...
            log_error("QuickTemplateError", str(e), "load_quick_template")
    ...
    def show_analytics(self):
        ...
        duration = (datetime.now() - self.analytics_data['start']).total_seconds()//60
        msg_rate = self.analytics_data['sent'] / (duration+1)
        error_rate = self.analytics_data['errors'] / (self.analytics_data['sent']+1)
        stats = f"Messages sent: {self.analytics_data['sent']}\nErrors: {self.analytics_data['errors']}\nSession duration: {duration} min\nMsg/min: {msg_rate:.2f}\nError rate: {error_rate:.2%}\n"
        # Randomly add more analytics
        if random.random() > 0.5:
            stats += f"Templates used: {len(self.quick_templates)}\n"
        if random.random() > 0.7:
            stats += f"Avg delay: {getattr(self, 'avg_delay', 1.0):.2f}s\n"
